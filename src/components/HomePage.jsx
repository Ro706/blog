...
function HomePage() {
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== 'All') {
      filtered = filtered.filter(blog => blog.tag.includes(selectedCategory));
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredBlogs.map((blog) => (
            <Link to={`/blog/${blog._id}`} key={blog._id} className="group block bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden">
              <div className="relative">
                <img src={blog.imageUrl || `https://source.unsplash.com/random/800x600?sig=${blog._id}`} alt={blog.title} className="w-full h-56 object-cover group-hover:scale-105 transition-transform duration-300" />
                <div className="absolute inset-0 bg-black/20"></div>
              </div>
              <div className="p-6">
                <div className="flex items-center mb-4">
                  <img src={blog.author.profilePicture} alt={blog.author.name} className="w-10 h-10 rounded-full object-cover" />
                  <div className="ml-3">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-3 group-hover:text-blue-500 transition-colors">{blog.title}</h2>
                <p className="text-gray-600 mb-4 h-20 overflow-hidden">{blog.description.substring(0, 120)}...</p>
